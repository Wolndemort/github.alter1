  ...
  const runAudioAction = async (action: "transcribe" | "isolate" | "process") => {
    ...
    try {
      if (action === "transcribe") {
        ...
        const blob = await api.audioAction(token, action === "isolate" ? "isolate" : "process", current.uri, action === "process" ? "Улучши качество записи и убери лишний шум" : "", current.filename || "alter-audio.m4a");
        await saveAudioResult(blob, action === "isolate" ? "isolated" : "processed");
      }
