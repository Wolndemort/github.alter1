...
export function ChatScreen({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  const startNewChat = async () => { if (busy) return; setBusy(true); setActivity("thinking"); try { await api.newSession(token); setItems([]); setMessage(""); setAttachment(null); setMenuVisible(false); resetIdle(); } catch (err) { setMenuError(err instanceof Error ? err.message : "Не удалось начать новый чат"); } finally { setBusy(false); setActivity(""); } };
...
const premiumStyles = StyleSheet.create({
  ...
  headerAction: { minWidth: 54, height: 40, alignItems: "center", justifyContent: "center" }, headerActionText: { color: "#d8ceff", fontSize: 22 }, newChatText: { color: "#d8ceff", fontSize: 15, fontWeight: "700", letterSpacing: 0.2 },
...
const mediaStyles = StyleSheet.create({
  ...
  attachButton: { width: 30, height: 38, alignItems: "center", justifyContent: "center" }, attachIcon: { color: "#d8ceff", fontSize: 23, lineHeight: 24, includeFontPadding: false, textAlign: "center" },
  voiceHalo: { width: 34, height: 38, alignItems: "center", justifyContent: "center" }, voiceHaloActive: { opacity: 0.72 }, voiceButton: { width: 34, height: 38, alignItems: "center", justifyContent: "center" }, voiceButtonActive: { opacity: 0.72 }, voiceIcon: { color: "#d8ceff", fontSize: 14, fontWeight: "800", letterSpacing: 0.8 },
  sendButton: { width: 34, height: 38, alignItems: "center", justifyContent: "center" }, sendIcon: { color: "#d8ceff", fontSize: 21, fontWeight: "700" },
});
