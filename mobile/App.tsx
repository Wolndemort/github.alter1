...
export function ChatScreen({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  return <SafeAreaView style={styles.container} onTouchStart={resetIdle}><Animated.View pointerEvents="none" style={[idleStyles.shade, { opacity: idleShade }]} /><KeyboardAvoidingView style={styles.chat} behavior={Platform.OS === "ios" ? "padding" : undefined}>
    <View style={styles.header}><Pressable style={[styles.menuButton, premiumStyles.menuButton]} onPress={() => { Keyboard.dismiss(); refreshAccount(); setMenuVisible(true); }} accessibilityLabel="Открыть боковую панель"><Text style={styles.menuIcon}>☰</Text></Pressable><Text style={styles.headerTitle}>ALTER</Text><Pressable style={[styles.menuButton, premiumStyles.newChatButton]} onPress={startNewChat} accessibilityLabel="Новый чат"><Text style={premiumStyles.newChatIcon}>＋</Text></Pressable></View>
...
const premiumStyles = StyleSheet.create({
  ...
  newChatButton: { backgroundColor: "#f7f4ff", borderColor: "#f7f4ff" }, newChatIcon: { color: "#17121f", fontSize: 23, lineHeight: 24 },
