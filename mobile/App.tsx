...
export function ChatScreen({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  return <SafeAreaView style={[styles.container, { backgroundColor: "#000000" }]} onTouchStart={resetIdle}>{idle ? <><Animated.View pointerEvents="none" style={[idleStyles.shade, { opacity: idleShade }]} /><IdleAlterScreen opacity={idleShade} /></> : null}<Pressable style={historyStyles.handle} onPress={() => setHistoryVisible(true)} accessibilityLabel="Открыть историю переписки"><Text style={historyStyles.arrow}>›</Text></Pressable><KeyboardAvoidingView style={styles.chat} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 8 : 0}>
   ...
   {(() => { const latestMedia = [...items].reverse().find((item) => item.mediaUri); return latestMedia ? <Pressable onPress={() => downloadMedia(latestMedia)} style={({ pressed }) => [mediaDownloadStyles.button, pressed && mediaDownloadStyles.pressed]} accessibilityLabel="Скачать последнее медиа"><Text style={mediaDownloadStyles.arrow}>↓</Text><View><Text style={mediaDownloadStyles.title}>Скачать последнее медиа</Text><Text style={mediaDownloadStyles.name}>{latestMedia.mediaFilename || latestMedia.mediaMime || "Медиафайл"}</Text></View></Pressable> : null; })()}
