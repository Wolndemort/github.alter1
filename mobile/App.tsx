...
type ChatItem = { id: string; role: string; text: string; createdAt?: number; mediaUri?: string; mediaUris?: { uri: string; mime: string; name?: string }[]; mediaMime?: string; mediaFilename?: string; audioUri?: string; audioMime?: string; audioFilename?: string; documentArtifactId?: string; feedback?: "positive" | "negative"; streaming?: boolean };
...
export function ChatScreen({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  const [imageAttachments, setImageAttachments] = useState<{ uri: string; mimeType?: string; filename?: string }[]>([]);
  ...
  useEffect(() => {
    api.history(token).then((result) => {
      ...
      // chat opens at the latest message without flashing from the top.
      initialHistoryScroll.current = true;
      autoScrollAfterUpdate.current = true;
  ...
  const saveAudioResult = async (blob: Blob, label: string): Promise<{ uri: string; filename: string }> => {
    ...
    const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onloadend = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(blob); });
    const comma = dataUrl.indexOf(",");
    const filename = `alter-${label}-${Date.now()}.mp3`;
    const uri = `${FileSystem.documentDirectory}${filename}`;
    await FileSystem.writeAsStringAsync(uri, comma >= 0 ? dataUrl.slice(comma + 1) : dataUrl, { encoding: FileSystem.EncodingType.Base64 });
    await Share.share({ url: uri, title: filename, message: Platform.OS === "android" ? uri : undefined });
    return { uri, filename };
  };
  const runAudioAction = async (action: "transcribe" | "process") => {
    if (!attachment || attachment.type !== "audio" || busy) return;
    const current = attachment; setBusy(true); setMenuError("");
    try {
      if (action === "transcribe") {
        const result = await api.speechToText(token, current.uri);
        setItems((old) => [...old, { id: `${Date.now()}t`, role: "assistant", text: result.text || "Не удалось получить расшифровку." }]);
      } else {
        const blob = await api.audioAction(token, "process", current.uri, "Улучши качество записи и убери лишний шум", current.filename || "alter-audio.m4a");
        await saveAudioResult(blob, "processed");
      }
      setAudioActionsVisible(false);
    } catch (err) { setMenuError(err instanceof Error ? err.message : "Не удалось обработать аудио"); }
    finally { setBusy(false); }
  };
  ...
  const send = async (presetText?: string) => {
    const text = (presetText ?? message).trim(); if ((!text && !attachment && !imageAttachments.length) || busy) return;
    ...
    autoScrollAfterUpdate.current = true;
    Keyboard.dismiss(); setMessage(""); setAttachment(null); setImageAttachments([]); setItems((old) => [...old, { id: userMessageId, role: "user", text: currentAttachment?.type === "audio" ? "Голосовое сообщение" : currentImages.length ? `${text || "Изображения"} · ${currentImages.length} шт.` : currentAttachment ? `${text || "Вложение"} · ${currentAttachment.type}` : text, mediaUris: currentImages.map((image) => ({ uri: image.uri, mime: image.mimeType || "image/jpeg", name: image.filename })) }, { id: pendingId, role: "assistant", text: "" }]); setBusy(true); setActivity(currentAttachment || currentImages.length ? "analyzing" : /найди|поищи|проверь|актуальн|новост|цену|погода/i.test(text) ? "searching" : /план|составь|распиши|подготовь|организуй/i.test(text) ? "planning" : "analyzing");
    try {
  ...
  const advanceWorkflow = async (complete = false) => {
    ...
    setWorkflowBusy(true); setMenuError("");
    try { setWorkflowData((await api.nextWorkflowStep(token, complete)).workflow); }
  ...
  const downloadMediaJob = async (job: MediaJob) => {
    ...
    try {
      ...
      await Share.share({ url: uri, title: filename, message: Platform.OS === "android" ? uri : undefined });
      Alert.alert("Готово", `Файл ${filename} подготовлен для скачивания.`);
    } catch (err) { setMenuError(userFacingError(err)); }
  };
  const createMediaJob = async () => {
    ...
    try { const kind = /(?:видео|ролик|клип|video|animation)/i.test(mediaJobPrompt) ? "video" : "image"; const job = await api.createMediaJob(token, kind, mediaJobPrompt.trim()); setMediaJobs((items) => [job, ...items]); setMediaJobPrompt(""); }
    catch (err) { setMediaJobsError(userFacingError(err)); }
    finally { setMediaJobsLoading(false); }
  };
  useEffect(() => {
  ...
  const createVoice = async () => {
    ...
    try {
      ...
      const previewAudio = result.previews?.[0]?.audio_base_64;
      const previewId = `${Date.now()}voice`;
      setItems((old) => [...old, { id: previewId, role: "assistant", text: "Голос создан и сохранён. Ниже можно прослушать или скачать preview. Для применения прикрепи голосовое и напиши: «измени мой голос на созданный».", ...(previewAudio ? { audioUri: `data:audio/mpeg;base64,${previewAudio}`, audioMime: "audio/mpeg", audioFilename: "alter-created-voice.mp3" } : {}) }]);
      if (previewAudio) await playAudioBase64(previewAudio, "mp3", previewId);
    } catch (err) { setMenuError(userFacingError(err)); }
    finally { setBusy(false); }
  };
  const pickMediaLibrary = async () => {
    ...
    try {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.All, quality: 0.85, allowsMultipleSelection: true, selectionLimit: 10 });
      console.info("ALTER picker media: result", { canceled: result.canceled, assets: result.assets?.length || 0 });
      if (!result.canceled && result.assets?.length) {
        const images = result.assets.filter((asset) => asset.type !== "video").map((asset) => ({ uri: asset.uri, mimeType: asset.mimeType || "image/jpeg", filename: asset.fileName || undefined })).slice(0, 10);
        const video = result.assets.find((asset) => asset.type === "video");
        if (video) setAttachment({ uri: video.uri, type: "video", mimeType: video.mimeType || "video/mp4", filename: video.fileName || undefined });
        else setImageAttachments((old) => [...old, ...images].slice(0, 10));
      }
  ...
  const startNewChat = async () => { if (busy || newChatLoading) return; setNewChatPromptVisible(false); setNewChatLoading(true); try { await api.newSession(token); setItems([]); setMessage(""); setAttachment(null); setImageAttachments([]); setMenuVisible(false); resetIdle(); } catch (err) { setMenuError(err instanceof Error ? err.message : "Не удалось начать новый чат"); } finally { setNewChatLoading(false); } };
  ...
  return <SafeAreaView style={[styles.container, { backgroundColor: "#000000" }]} onTouchStart={resetIdle}>{idle ? <><Animated.View pointerEvents="none" style={[idleStyles.shade, { opacity: idleShade }]} /><IdleAlterScreen opacity={idleShade} /></> : null}<Pressable style={historyStyles.handle} onPress={() => setHistoryVisible(true)} accessibilityLabel="Открыть историю переписки"><Text style={historyStyles.arrow}>›</Text></Pressable><KeyboardAvoidingView style={styles.chat} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 8 : 0}>
    ...
    {imageAttachments.length ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 12, gap: 8, paddingBottom: 4 }}>{imageAttachments.map((image, index) => <View style={mediaStyles.attachmentThumb} key={`${image.uri}-${index}`}><Image source={{ uri: image.uri }} style={mediaStyles.attachmentThumbImage} /><Pressable onPress={() => setImageAttachments((old) => old.filter((_, itemIndex) => itemIndex !== index))} accessibilityLabel={`Удалить изображение ${index + 1}`} style={mediaStyles.attachmentThumbRemove}><Text style={mediaStyles.removeAttachment}>×</Text></Pressable></View>)}</ScrollView> : null}
    {attachment ? <View style={mediaStyles.attachmentChip}>{attachment.type === "image" ? <Image source={{ uri: attachment.uri }} style={{ width: 42, height: 42, borderRadius: 8, marginRight: 8 }} /> : null}<Text style={mediaStyles.attachmentText}>{attachment.filename || (attachment.type === "audio" ? "Голосовое сообщение" : attachment.type === "video" ? "Видео прикреплено" : "Файл прикреплён")}</Text>{attachment.type === "audio" ? <Pressable onPress={() => setAudioActionsVisible(true)} disabled={busy} accessibilityLabel="Действия с аудио"><Text style={mediaStyles.generateAction}>♫ Действия</Text></Pressable> : attachment.type === "document" ? <Pressable onPress={() => setDocumentEditVisible(true)} disabled={busy} accessibilityLabel="Редактировать документ"><Text style={mediaStyles.generateAction}>✎ Изменить</Text></Pressable> : <Pressable onPress={generateAttachment} disabled={busy} accessibilityLabel="Изменить вложение"><Text style={mediaStyles.generateAction}>✦ Изменить</Text></Pressable>}<Pressable onPress={() => setAttachment(null)} accessibilityLabel="Удалить вложение"><Text style={mediaStyles.removeAttachment}>×</Text></Pressable></View> : null}
  ...
  <Modal visible={menuVisible && !memoryVisible && !remindersVisible && !faqVisible && !plansVisible} transparent animationType="none" onRequestClose={() => setMenuVisible(false)}>
    <Pressable style={premiumStyles.drawerBackdrop} onPress={() => setMenuVisible(false)}>
      ...
      <Pressable style={premiumStyles.drawerContent} onPress={(event) => event.stopPropagation()}>
        ...
        <Pressable style={premiumStyles.menuAction} onPress={() => void safeOpenUrl(`mailto:kid.cudi.1995@mail.ru?subject=${encodeURIComponent("Поддержка ALTER")}&body=${encodeURIComponent(`Опиши проблему подробно.\n\nАккаунт: ${account?.email || "не определён"}\nУстройство: mobile`)}`)}><Text style={premiumStyles.menuActionText}>Поддержка</Text><Text style={premiumStyles.menuActionArrow}>?</Text></Pressable>
  ...
  <Modal visible={mediaJobsVisible} animationType="slide" onRequestClose={() => setMediaJobsVisible(false)}>
    <SafeAreaView style={styles.memoryScreen}><View style={styles.memoryHeader}><Text style={styles.memoryTitle}>Медиа-задачи</Text><Pressable style={premiumStyles.menuAction} onPress={() => setMediaJobsVisible(false)}><Text style={premiumStyles.menuActionText}>Назад</Text><Text style={premiumStyles.menuActionArrow}>→</Text></Pressable></View>
      <View style={{ paddingHorizontal: 20, paddingBottom: 12 }}><TextInput value={mediaJobPrompt} onChangeText={setMediaJobPrompt} placeholder="Что создать? Например: кинематографичное видео города" placeholderTextColor="#777" multiline style={styles.voiceDescriptionInput} /><Text style={styles.myDayIntro}>Медиа — изображения и видео сохраняются вместе</Text><Pressable style={[permissionStyles.primary, { opacity: mediaJobPrompt.trim() && !mediaJobsLoading ? 1 : 0.45, marginTop: 8 }]} onPress={createMediaJob} disabled={!mediaJobPrompt.trim() || mediaJobsLoading}><Text style={permissionStyles.primaryText}>Запустить задачу</Text></Pressable></View>
  ...
  <NotificationModal visible={notificationsVisible} items={notifications} unread={notificationUnread} onClose={() => setNotificationsVisible(false)} onRead={markNotificationRead} onReadAll={markAllNotificationsRead} onOpenChat={(item) => { setNotificationsVisible(false); setMessage(item.body); }} />
...
const planStyles = StyleSheet.create({
  ...
  card: { backgroundColor: "transparent", borderBottomWidth: 1, borderBottomColor: "#303030", paddingVertical: 12, gap: 5 },
  featured: { borderBottomColor: "#777", shadowOpacity: 0, elevation: 0 },
...
const mediaStyles = StyleSheet.create({
  ...
  attachmentThumb: { width: 58, height: 58, borderRadius: 9, overflow: "hidden", position: "relative", backgroundColor: "#1d1d1d" }, attachmentThumbImage: { width: 58, height: 58 }, attachmentThumbRemove: { position: "absolute", top: 2, right: 2, width: 20, height: 20, borderRadius: 10, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,0.72)" },
  ...
  sendButton: { width: 34, height: 38, alignItems: "center", justifyContent: "center" }, stopSendButton: { backgroundColor: "#2b2538", borderRadius: 17 }, stopAudioButton: { marginHorizontal: 12, marginBottom: 4, paddingVertical: 8, alignItems: "center", borderRadius: 10, backgroundColor: "#1d1d1d" }, sendIcon: { color: "#ffffff", fontSize: 21, fontWeight: "700" }, inputMask: { display: "none" }, inputGlow: { height: 0.5, backgroundColor: "#ffffff", opacity: 0.72, shadowColor: "#ffffff", shadowOpacity: 0.45, shadowRadius: 4, elevation: 2 },
});

// ALTER palette: all product accents remain strictly black, white and neutral gray.
(styles as Record<string, unknown>).workflowCard = { ...StyleSheet.flatten(styles.workflowCard), backgroundColor: "#151515", borderColor: "#555" };
(styles as Record<string, unknown>).workflowKicker = { ...StyleSheet.flatten(styles.workflowKicker), color: "#fff" };
(styles as Record<string, unknown>).workflowAction = { ...StyleSheet.flatten(styles.workflowAction), color: "#fff" };
(styles as Record<string, unknown>).memoryAuditCard = { ...StyleSheet.flatten(styles.memoryAuditCard), backgroundColor: "#1a1a1a", borderColor: "#666" };
(styles as Record<string, unknown>).memoryAuditTitle = { ...StyleSheet.flatten(styles.memoryAuditTitle), color: "#fff" };
(styles as Record<string, unknown>).memoryAuditItem = { ...StyleSheet.flatten(styles.memoryAuditItem), color: "#ddd" };
(premiumStyles as Record<string, unknown>).sectionLabel = { ...StyleSheet.flatten(premiumStyles.sectionLabel), color: "#fff" };
(premiumStyles as Record<string, unknown>).drawerError = { ...StyleSheet.flatten(premiumStyles.drawerError), backgroundColor: "#211719", borderColor: "#666" };
(premiumStyles as Record<string, unknown>).drawerErrorText = { ...StyleSheet.flatten(premiumStyles.drawerErrorText), color: "#eee" };
(premiumStyles as Record<string, unknown>).dangerAction = { ...StyleSheet.flatten(premiumStyles.dangerAction), borderColor: "#666" };
(linkStyles as Record<string, unknown>).link = { ...StyleSheet.flatten(linkStyles.link), color: "#fff" };
(mediaStyles as Record<string, unknown>).stopSendButton = { ...StyleSheet.flatten(mediaStyles.stopSendButton), backgroundColor: "#333" };
(styles as Record<string, unknown>).clearMemoryButton = { ...StyleSheet.flatten(styles.clearMemoryButton), borderColor: "#666" };
(styles as Record<string, unknown>).clearMemoryText = { ...StyleSheet.flatten(styles.clearMemoryText), color: "#eee" };
(permissionStyles as Record<string, unknown>).body = { ...StyleSheet.flatten(permissionStyles.body), color: "#ccc" };
(permissionStyles as Record<string, unknown>).primary = { ...StyleSheet.flatten(permissionStyles.primary), backgroundColor: "#fff" };
