...
ALTER — мультимодальный ассистент: текст, голос, интернет-поиск с источниками, изображения, видео, аудио, OCR и документы. Документы можно анализировать и передавать многошаговому агенту; изменённые TXT, Markdown, CSV, JSON и DOCX возвращаются как файлы. Агент строит планы на любой горизонт и работает с задачами, напоминаниями, push и календарём.

Подробная карта: `docs/CAPABILITIES_AND_API.md`.

## Feedback и персональное улучшение

Оценки 👍/👎 доступны в Telegram, web и mobile. Они сохраняются как ограниченные по размеру персональные примеры и подмешиваются в контекст ответа как рекомендации, а не как автоматическое переобучение модели. Каждые 72 часа при включённом proactive-режиме ALTER может прислать мини-опрос о полезности последнего ответа. Контур feedback не хранит внутренние инструкции и отбрасывает небезопасные ответы.
\n+## Создание заполненных документов
\n+ALTER умеет создавать файл с нуля и сразу заполнять его содержимым. Поддерживаются DOCX, PDF, XLSX, PPTX, ODT, RTF, TXT, Markdown, CSV и JSON.
\n+Примеры: «создай TXT-файл и напиши в нём план запуска проекта на 30 дней»; «создай заполненный DOCX с коммерческим предложением»; «сформируй XLSX-таблицу бюджета с категориями и итогами».
\n+Команда «создай TXT» создаёт файл, но для содержательного результата нужно сразу указать тему, структуру и данные. Незавершённый запрос на напоминание хранится 60 минут; после этого новое сообщение обрабатывается как обычный диалог.
