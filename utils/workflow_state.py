...
def suggest_steps(goal: str, workflow_id: str = "finish_task") -> list[str]:
    """Create useful deterministic steps without an extra model round-trip."""
    ...
    if any(word in value.casefold() for word in ("проект", "запуск", "продукт", "лендинг")):
...
def start_workflow(settings: dict | None, workflow_id: str, goal: str, steps=None) -> dict:
    ...
    state = {
        "id": str(workflow_id)[:48],
        "goal": str(goal).strip()[:500],
        "steps": normalized_steps or list(DEFAULT_STEPS),
        "current_step": 0,
        "status": "active",
...
def advance_workflow(settings: dict | None, complete: bool = False) -> dict:
    ...
    if complete or current >= len(steps) - 1:
        state["status"] = "completed" if complete else "ready_for_review"
    value[WORKFLOW_KEY] = state
    return value
...
def workflow_view(settings: dict | None) -> dict | None:
    ...
    result["current_step"] = index
    result["current_step_title"] = steps[index]
