...
async def generate_media_reply(
    ...
    try:
        ...
        messages = [{"role": "system", "content": MEDIA_SYSTEM_PROMPT + "\n\n" + MEMORY_POLICY_PROMPT + "\n\n" + PUBLIC_RESPONSE_POLICY + context_note + "\n\n" + (
            "Это не запрос на подпись к фотографии. Воспринимай текущее сообщение "
            "как очередную реплику одного продолжающегося разговора. Сначала определи, "
            "какой результат просит пользователь, затем дай именно этот результат, "
            "опираясь на изображение и предыдущие реплики. Если вопрос связан с "
            "предыдущей темой, продолжи её и не начинай описание изображения с нуля. "
            "Не отвечай фразами вроде «на фото всё ясно», «это изображение» или "
            "«вижу скриншот» без конкретного ответа. Для разбора, сравнения или "
            "совета назови вывод, 2–4 ключевых основания и практический следующий "
            "шаг, если он уместен. Если пользователь не задал явный вопрос, сам "
            "сформулируй полезный краткий разбор содержимого, а не пустое описание. "
            "Материал уже передан как бинарное вложение. Не обсуждай base64, кодировку, "
            "URL или формат передачи; анализируй содержимое. Если оно действительно "
            "нечитаемо, назови конкретную причину."
        )}]
...
async def extract_visual_context(
    ...
    content = [{
        "type": "text",
        "text": (
            "Верни только JSON без markdown. Проведи точный visual audit: опиши "
            "объекты, сцену, читаемый текст/OCR, цвета, стиль, посадку, геометрию, "
            "табличные данные и важные детали. Не определяй личность, возраст, бренд, "
            "цену или настроение без надёжных визуальных оснований. Отделяй видимое "
            "от предположений. Формат: {\"scene\":\"\", \"items\":[], "
            "\"ocr_text\":\"\", \"colors\":[], \"style\":[], \"fit\":[], "
            "\"measurements\":[], \"table\":[], \"details\":[], \"uncertainty\":[]}. "
            "Запрос пользователя: "
