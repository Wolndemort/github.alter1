...
«Хочу ссылку на трейлер Cyberpunk 2077»
«Скачай видео по Cyberpunk 2077»

Готовые изображения и видео:
«Скинь карту из Cyberpunk 2077»
«Найди фотографию Эйфелевой башни»
«Найди видео по Cyberpunk и пришли файл»

ALTER ищет готовые материалы в интернете и отправляет их в чат. Для YouTube-видео сначала пытается подготовить скачиваемый MP4; если ролик слишком большой или скачать его нельзя, отправляет ссылку.
...
Чтобы создать и сразу заполнить файл, формулируй запрос одной командой: «создай TXT-файл и напиши в нём план запуска проекта на 30 дней» или «создай заполненный DOCX с коммерческим предложением». Если указать только «создай txt», ALTER создаст файл, но для полезного результата лучше сразу описать тему, структуру и нужные данные.

Незавершённый запрос на напоминание действует 60 минут. Если вернуться в чат через несколько часов, новое сообщение не будет принято за ответ на старое напоминание.

Обратная связь и улучшение: после каждого ответа можно нажать 👍 или 👎 в web, mobile или Telegram. ALTER сохраняет только короткий безопасный пример ответа и использует повторяющиеся оценки как персональные рекомендации по стилю и полезности. Раз в 72 часа может прийти короткий мини-опрос; его можно пропустить, отключив proactive_enabled.
"""
